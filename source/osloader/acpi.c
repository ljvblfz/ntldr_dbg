    ...
    )
{
    for (ULONG i = 0; i < 2; i++) {
        ...
        if (i == 0) {
            MapStart.QuadPart = 0;
            PUSHORT FirstPage = (PUSHORT)(MmMapIoSpace(MapStart, 0x1000, MmNonCached));
            ...
            if (FirstPage[0x40e / sizeof(USHORT)]) {
                ...
                Start = (PULONGLONG)(MmMapIoSpace(MapStart, 0x2000, MmNonCached));
                End = Start + 0x400 / sizeof(ULONGLONG);
            }
        } else {
            MapStart.QuadPart = 0xe0000;
            Start = (PULONGLONG)(MmMapIoSpace(MapStart, 0x1ffff, MmNonCached));
        ...
        for (; Start < End; Start++) {
            if (*Start == ACPI_RSDP_SIGNATURE) {
                ...
                if (!Checksum) {
                    ...
                    BlRsdt = (PACPI_RSDT)(MmMapIoSpace(MapStart, sizeof(ACPI_DESCRIPTION_HEADER), MmNonCached));

                    //
                    // map the whole table
                    //
                    BlRsdt = (PACPI_RSDT)(MmMapIoSpace(MapStart, BlRsdt->Header.Length, MmNonCached));
    ...
    )
{
    ...
    if (Signature == ACPI_DSDT_SIGNATURE) {
        ...
        // map it
        //
        return (PACPI_DESCRIPTION_HEADER)(MmMapIoSpace(MapAddress, Length, MmNonCached));
    }
    ...
    for (ULONG i = 0; i < Count; i++) {
        ...
        // map header
        //
        PACPI_DESCRIPTION_HEADER Header = (PACPI_DESCRIPTION_HEADER)(MmMapIoSpace(MapAddress, sizeof(ACPI_DESCRIPTION_HEADER), MmNonCached));
        ...
        if (Header && (Header->Signature == Signature)) {
            ...
            //
            if (PAGE_ALIGN(Add2Ptr(Header, Length, PVOID)) <=
                PAGE_ALIGN(Add2Ptr(Header, sizeof(ACPI_DESCRIPTION_HEADER), PVOID))) {
                return Header;
            }
...
BlRetrieveBIOSRedirectionInformation (
    __inout PLOADER_REDIRECTION_INFORMATION Info
    )
{
    //
    // find spcr table
    ...
    )
{
    ...
    //
    PUCHAR Start = (PUCHAR)(0xe0000);
    PUCHAR End = (PUCHAR)(0x100000);
