        ...
        while ( 1 ) {
            ...
            if ( BootDeviceName ) {
                ...
                if ( !AlreadyInitialized ) {
                    ...
                    //
                    // from now on, no one can change firmware's
                    // permanent heap,because we also use it to save config tree
                    //
...
BlGetActivePartition(
    __out PCHAR BootPartitionName,
    __in ULONG BootDrive,
    __in ULONG Partition
    )
{
    //
...
DoGlobalInitialization (
    __in PBOOT_CONTEXT BootContextRecord
    )
{
    //
...
VOID
ParseCommandLine (
    __in PCHAR CommandLine,
    __in PBOOT_CONTEXT BootRecord
    )
{
    //
    // parse root device
    ...
    if ( RootDevice ) {
        ...
        RootDevice += 10;
        if ( !strncmp( RootDevice, "HD", 2 ) ||
             !strncmp( RootDevice, "SD", 2 ) ) {
            //
            // skip HD,SD
            //
...
//
// entry pointer, su module will call it with page disabled,
// pe enabled, interrupt off
//
VOID
NtProcessStartup (
    __in PBOOT_CONTEXT BootContextRecord
    )
{
    ...
    if ( BootContextRecord->FSContextPointer->BootDrive == 0x00 ||
         BootContextRecord->FSContextPointer->BootDrive == 0x01 ) {
        //
        // booting from A:,B:
        //
    ...
    // the OS loader heap, and the OS loader parameter block.
    //
