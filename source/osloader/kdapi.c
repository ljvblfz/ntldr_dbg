...
{
    PUCHAR DstBuffer = (PUCHAR)(Dest);
    PUCHAR SrcBuffer = (PUCHAR)(Src);
    ...
    )
{
    ...
    while (Add2Ptr(SrcBuffer, 0, ULONG) & 3) {
        ...
        PUCHAR WriteBuffer = (PUCHAR)(BdWriteCheck(DestBuffer));
        PUCHAR ReadBuffer = (PUCHAR)(BdWriteCheck(SrcBuffer));
    ...
    while (MoveLength > 3) {
        PULONG WriteBuffer = (PULONG)(BdWriteCheck(DestBuffer));
        PULONG ReadBuffer = (PULONG)(BdWriteCheck(SrcBuffer));
        if (!WriteBuffer || !ReadBuffer) {
            break;
        }
    ...
    )
{
    ...
    if (BdMoveMemory((PVOID)(BdBreakpointTable[Index].Address), &BdBreakpointTable[Index].Content, sizeof(UCHAR)) != sizeof(UCHAR)) {
...
{
    UCHAR OldByte;
    BOOLEAN AddressValid = BdMoveMemory(&OldByte, (PVOID)(Address), sizeof(OldByte)) == sizeof(OldByte);

    if (AddressValid) {
        if (!BdWriteCheck((PVOID)(Address))) {
            return 0;
        }
    }
    ...
    if (AddressValid) {
        ...
        BdMoveMemory((PVOID)(Address), &BdBreakpointInstruction, sizeof(BdBreakpointInstruction));
    } else {
    ...
    )
{
    ...
    if (!(BdBreakpointTable[Index].Flags & KD_BREAKPOINT_SUSPENDED) ||
          BdBreakpointTable[Index].Flags & KD_BREAKPOINT_NEEDS_REPLACE) {
...
BdDeleteBreakpointRange (
    __in ULONGLONG Lower,
    __in ULONGLONG Upper
    )
{
    //
    ...
    for (ULONG Index = 0; Index < BREAKPOINT_TABLE_SIZE; Index++) {
        if ((BdBreakpointTable[Index].Flags & KD_BREAKPOINT_IN_USE) &&
...
VOID
BdSetContextState (
    __inout PDBGKD_WAIT_STATE_CHANGE64 WaitStateChange,
    ...
    )
{
    ...
    // sign extend
    //
    WaitStateChange->ProgramCounter     = (ULONG64)((LONG)(Context->Eip));
    ...
    WaitStateChange->ControlReport.InstructionCount =
        (USHORT)(
            BdMoveMemory(WaitStateChange->ControlReport.InstructionStream,
                         Add2Ptr(Context->Eip, 0, PVOID),
                         DBGKD_MAXSTREAM));
    ...
    if (BdDeleteBreakpointRange(Context->Eip, WaitStateChange->ControlReport.InstructionCount + Context->Eip - 1)) {
        BdMoveMemory(WaitStateChange->ControlReport.InstructionStream,
                     Add2Ptr(Context->Eip, 0, PVOID),WaitStateChange->ControlReport.InstructionCount);
    }
}
...
ExceptionRecord32To64 (
    __in PEXCEPTION_RECORD ExceptionRecord,
    __out PEXCEPTION_RECORD64 ExceptionRecord64
    )
{
    //
    // sign extend
    //
    ExceptionRecord64->ExceptionAddress = (ULONG64)((LONG)(ExceptionRecord->ExceptionAddress));
    ...
    ExceptionRecord64->ExceptionRecord  = (ULONG)(ExceptionRecord->ExceptionRecord);
    ...
    for (ULONG i = 0; i < ARRAYSIZE(ExceptionRecord->ExceptionInformation); i++) {
        ExceptionRecord64->ExceptionInformation[i] = (ULONG64)((LONG_PTR)(ExceptionRecord->ExceptionInformation[i]));
    }
}
    ...
    )
{
    ...
    MessageHeader.Length = (USHORT)(sizeof(DBGKD_MANIPULATE_STATE64));
    MessageHeader.Buffer = Add2Ptr(m, 0, PCHAR);
    PVOID Address = (PVOID)(m->WriteMemory.TargetBaseAddress);
    ...
    )
{
    ...
    if ((ULONG)(a->TargetBaseAddress) + Length <= sizeof(KPROCESSOR_STATE)) {
        extern KPRCB BdPrcb;
        BdCopyMemory(AdditionalData->Buffer, Add2Ptr(&BdPrcb.ProcessorState.ContextFrame.ContextFlags, (ULONG)(a->TargetBaseAddress), PVOID), Length);
    ...
    )
{
    ...
    if ((ULONG)(a->TargetBaseAddress) + Length <= sizeof(KPROCESSOR_STATE)) {
        extern KPRCB BdPrcb;
        BdCopyMemory(Add2Ptr(&BdPrcb.ProcessorState.ContextFrame.ContextFlags, (ULONG)(a->TargetBaseAddress), PVOID), AdditionalData->Buffer, Length);
        m->ReturnStatus = STATUS_SUCCESS;
    ...
    )
{
    ...
    switch (a->DataSize) {
    case 1:
        a->DataValue = READ_PORT_UCHAR((PUCHAR)(a->IoAddress));
        break;
    ...
    case 2:
        if (a->IoAddress & 1) {
            ...
            a->DataValue = READ_PORT_USHORT((PUSHORT)(a->IoAddress));
        }
        break;
    ...
    )
{
    ...
    switch (a->DataSize) {
    case 1:
        WRITE_PORT_UCHAR((PUCHAR)(a->IoAddress), (UCHAR)(a->DataValue & 0xff));
        break;
    ...
    case 2:
        if (a->IoAddress & 1) {
            ...
            WRITE_PORT_USHORT((PUSHORT)(a->IoAddress), (USHORT)(a->DataValue & 0xffff));
        }
        break;
    ...
    )
{
    ...
    MessageHeader.Length = (USHORT)(sizeof(DBGKD_MANIPULATE_STATE64));
    MessageHeader.Buffer = Add2Ptr(m, 0, PCHAR);
    ULONG StartAddress = (ULONG)(a->TargetBaseAddress);
    ...
    if (PAGE_ALIGN(StartAddress) == PAGE_ALIGN(EndAddress)) {
        ...
        if (Address) {
            AdditionalData->Length = (USHORT)(BdMoveMemory(AdditionalData->Buffer, Address, ReadLength));
        } else {
            AdditionalData->Length = 0;
        }
    } else {
        do {
            ...
            AdditionalData->Length += (USHORT)(BdMoveMemory(Buffer, Address, LengthThisRun));
    ...
    )
{
    ...
    MessageHeader.Buffer = Add2Ptr(m, 0, PCHAR);
    ULONG StartAddress = (ULONG)(a->TargetBaseAddress);
    ULONG EndAddress = StartAddress + (USHORT)(a->TransferCount);
    ULONG Count = 0;
    ...
    )
{
    ...
    ManipulateState->GetVersion64.MajorVersion = (USHORT)(0x400 | (NtBuildNumber >> 0x1c));
    ManipulateState->GetVersion64.MinorVersion = (USHORT)(NtBuildNumber & 0x0000ffff);
    ...
    ManipulateState->GetVersion64.PsLoadedModuleList = (ULONG64)(&BdModuleList);
    ManipulateState->GetVersion64.KernBase = (ULONG64)(BdModuleDataTableEntry.DllBase);
...
BdSendWaitContinue (
    ...
    __in PSTRING OutputHead,
    __in PSTRING OutputData,
    __in PCONTEXT Context
    )
{
    STRING InputHead;
    ...
    )
{
    ...
    //
    STRING MessageHeader;
    MessageHeader.Length = (USHORT)(sizeof(DBGKD_DEBUG_IO));
...
BOOLEAN
BdPromptString (
    __in PSTRING InputString,
    __in PSTRING OutputString
    )
{
    //
    // move the output string to the message buffer.
    ...
    OutputString->Length = (USHORT)(BdMoveMemory(OutputString->Buffer,BdMessageBuffer,Length));
...
NTSTATUS
BdCreateRemoteFile (
    __out PULONG RemoteFileId,
    __out PULONG64 FileSize,
    __in PCHAR FileName,
    ...
    )
{
    ...
    UnicodeFileNameString.Buffer = (PWCHAR)((PVOID)(BdFileTransferBuffer));
    ...
    MessageHeader.Buffer        = (PCHAR)((PVOID)(&FileIo));

    STRING MessageData;
    MessageData.Length = (USHORT)(strlen(FileName) * sizeof(WCHAR) + sizeof(WCHAR));
...
NTSTATUS
BdReadRemoteFile (
    __in ULONG RemoteFileId,
    __in ULONG64 Offset,
    __out PVOID Buffer,
    __in ULONG Length,
    __out PULONG ActualCount
    )
{
    *ActualCount = 0;
    ...
    while (Length) {
        ...
        //
        STRING MessageData;
        MessageData.Buffer = (PCHAR)(Buffer);
        MessageData.MaximumLength = (USHORT)(FileIo.ReadFile.Length);
    ...
    )
{
    ...
    __try {
        ...
        // allocate memory
        //
        ULONG Count = (ULONG)((FileSize + PAGE_SIZE - 1) >> PAGE_SHIFT);
        ...
        //
        ULONG64 Offset = 0;
        PVOID Buffer = (PVOID)(BasePage << PAGE_SHIFT);
        while (FileSize > Offset) {
            ...
            //
            ULONG ActualSize = 0;
            Status = BdReadRemoteFile(RemoteFileId, Offset, Buffer, (ULONG)(ReadSize), &ActualSize);
        ...
        BlFileTable[FileId].u.NetFileContext.FileSize   = (ULONG)(FileSize);
        BlFileTable[FileId].u.NetFileContext.FileBuffer = (PVOID)(BasePage << PAGE_SHIFT);
