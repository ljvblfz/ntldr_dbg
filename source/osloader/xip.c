    ...
    )
{
    ...
    PHARDWARE_PTE Pte = (PHARDWARE_PTE)(PDE_BASE_X86) + (VirtualAddress >> 22);
    ULONG i = 0;
    ...
    while ((Status == ESUCCESS) && PageCount) {
        ...
        PVOID MappedAddress = (PVOID)(VirtualAddress);
        ...
        for (ULONG i = 0; i < 4 * 1024 * 1024 / (8 * 1024); i++) {
            ...
            if (!FileOffset || (ReadCount >= 8 * 1024)) {
                ...
                //
                if ((Bpb->BytesPerSector != SECTOR_SIZE) ||
                    (Bpb->SectorsPerCluster != PAGE_SIZE / SECTOR_SIZE)) {
                    //
                    // invalid format, exit
                ...
                if (AdjustedSize < PAGE_SIZE) {
                    Bpb->ReservedSectors += (USHORT)(AdjustedSize >> SECTOR_SHIFT);
                }
